...
contract Lottery is OwnableUpgradeable {
    ...
    function buy(uint256 _price, uint8[4] memory _numbers) external {
        ...
        reward.safeTransferFrom(address(msg.sender), address(this), _price);
    ...
    function multiBuy(uint256 _price, uint8[4][] memory _numbers) external {
        ...
        reward.safeTransferFrom(address(msg.sender), address(this), totalPrice);
    ...
    function claimReward(uint256 _tokenId) external {
        ...
        if(reward>0) {
            safeRewardTransfer(address(msg.sender), reward);
        }
    ...
    function multiClaim(uint256[] memory _tickets) external {
        ...
        if(totalReward>0) {
            safeRewardTransfer(address(msg.sender), totalReward);
        }
    ...
    // Safe reward transfer function, just in case if rounding error causes pool to not have enough EGGs.
