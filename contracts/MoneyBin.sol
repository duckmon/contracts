...
// This is Scrooges Money Bin, it's packed with lots of pennies that will bring you luck.
// Take a dive and swim with Scrooge.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract MoneyBin is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                uint256 scroogeFee = depositFee.mul(scroogeCut).div(100);

                pool.lpToken.safeTransfer(feeAddress, depositFee.sub(scroogeCut));
                pool.lpToken.safeTransfer(devaddr, scroogeFee);
    ...
    function scrooge(uint256 cut) public onlyOwner {
        require(cut < 100, "scrooge: cut cannot exceed 100");
        require(cut >= 0, "scrooge: cut cannot be negative");
        scroogeCut = cut;
        emit Scrooge(cut);
    }
